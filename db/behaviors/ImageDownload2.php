...
class ImageDownload2 extends Behavior
{
    ...
    protected function processImageDownload(array &$newAttributes = [])
    {
        ...
        if ($owner instanceof ActiveRecord) {
            ...
            if (is_array($primaryKey)) {
                ...
                $basenamePrefix = substr($primaryKey, -2);
            }
            $basenamePrefix = str_pad($basenamePrefix, 2, '0', STR_PAD_LEFT);
            foreach ($this->buildAttributes() as $sourceAttributeName => $destAttributes) {
                if ($owner->{$sourceAttributeName} && is_string($owner->{$sourceAttributeName}) && preg_match('~^(https?\://[^\s]+)(?:\s(\d+))?$~i', $owner->{$sourceAttributeName}, $match) && ((count($match) == 2) || ($match[2] != 200))) {
                    $url = $match[1];
                    $curl = Curl::init([
                        'url' => $url,
                        'isTempFilename' => true,
